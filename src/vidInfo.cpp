...
#define SMALL 192, 256
    ...
    void VideoInfo::create_file(const char* vid_filename, const char* newV_filename, unsigned char* img_buf, int& img_width, int& img_height) {
        ...
        unsigned char* vid_info = new unsigned char[file_size]; //video infoemation
        fread(vid_info, sizeof(unsigned char), file_size, video_file); //reading video info
        fclose(video_file);

        std::vector<std::thread> threads; //threads vector

        for (i = 0; i < file_size; i += frame_size) { //frame by frame cycle 
